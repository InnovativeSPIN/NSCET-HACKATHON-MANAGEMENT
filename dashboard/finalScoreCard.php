...
<body class="g-sidenav-show   bg-gray-100">
  ...
  <main class="main-content position-relative border-radius-lg ">
    ...
    <div class="form-container">
        <form action="../controllers/final_scorecard_submit.php" id="problem-form" method="post" class="form-content">
          <h3>Select Problem Statement</h3>
              ...
              <!-- New fields to display team details and mentor name -->
              <h3>Team Details</h3>
              <div class="form-group">
    ...
    <h2>Final Evaluation Scorecard</h2>
    ...
    <form>
        ...
        <div class="total-score">
            TOTAL SCORE: <input type="number" name="total_score" readonly> / 100
        </div>
  ...
  <script>
    function fetchTeams() {
      ...
      function fetchTeamDetails() {
          var teamId = document.getElementById('team_id').value; // Get the value from the passed element

          
          if (teamId) {
              // Fetch team details based on selected team_id
              var xhr = new XMLHttpRequest();
              xhr.open('POST', '../utils/fetch_team_details.php', true);
              xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
              xhr.onload = function () {
                  if (xhr.status === 200) {
                      ...
                      if (response.team_name) {
                          document.getElementById('team_name').value = response.team_name;
                          var mentorId = response.mentor_id;
                          if (mentorId) {
                              // Fetch mentor name based on mentor_id
      ...
      function fetchMentorName(mentorId) {
          var xhr = new XMLHttpRequest();
          xhr.open('POST', '../utils/fetch_mentor_name.php', true);
          xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
          xhr.onload = function () {
              if (xhr.status === 200) {
                  var response = JSON.parse(xhr.responseText);
                  ...
                  console.log(response)
                  if (response.name) {
                      document.getElementById('mentor_name').value = response.name;
                  }
              }
          };
