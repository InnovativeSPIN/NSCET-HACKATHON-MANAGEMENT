...
<?php
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo '      <button class="select-btn" onclick="showConfirmation(event, \'' . htmlspecialchars($row['name']) . '\')">Select</button>';
            echo '    </div>';
            ...
            echo '      <p><strong>Area of Interest:</strong> ' . htmlspecialchars($row['area_of_interest']) . '</p>';
            echo '    </div>';
            echo '  </div>';
            echo '</li>';
        }
